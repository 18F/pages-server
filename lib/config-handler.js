...
ConfigHandler.prototype.init = function() {
  return Promise.all([
    checkInternalPublishingConfiguration(this),
    checkForBundler(this)
  ]);
};

function checkInternalPublishingConfiguration(handler) {
  return handler.fileHandler.exists('_config_internal.yml')
...
function checkForBundler(handler) {
  return handler.fileHandler.exists('Gemfile')
    .then(function(usesBundler) {
      handler.usesBundler = usesBundler;
    });
}
