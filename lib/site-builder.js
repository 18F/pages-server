...
SiteBuilder.prototype.jekyllBuild = function() {
  ...
  args = args.concat(['build', '--trace', '--destination']);

  if (!this.hasInternalConfig) {
    return this.spawn(jekyll, args.concat(
      [this.buildDestination,
       '--config', '_config.yml,' + this.opts.pagesConfig]));
  }

  var buildPromise = this.spawn(jekyll, args.concat(
    [this.internalBuildDestination,
     '--config', '_config.yml,_config_internal.yml,' + this.opts.pagesConfig]));
  var externalConfigs = '_config.yml,';
  if (this.hasExternalConfig) { externalConfigs += '_config_external.yml,'; }

  var that = this;
  return buildPromise.then(function() {
    that.spawn(jekyll, args.concat(
      [that.buildDestination,
       '--config', externalConfigs + that.opts.pagesConfig]));
  });
};
