...
SiteBuilder.prototype.finishBuild = function(err) {
  var builder = this,
      logger = this.logger,
      opts = this.opts;
...
SiteBuilder.prototype.build = function() {
  ...
  doBuild = function() {
    ...
    finish = function(err) {
      return builder.finishBuild(err);
    };

    return builder.gitRunner.prepareRepo(builder.branch)
      .then(function() { return builder.fileHandler.exists('_config.yml'); })
...
function rsync(builder) {
  return builder.commandRunner.run(config.rsync,
    config.rsyncOpts.concat(['./', builder.buildDestination]));
}

function buildJekyll(builder) {
  return builder._checkInternalPublishingConfiguration()
    .then(function() {
    ...
    .then(function() {
      return builder.fileHandler.exists(builder.opts.pagesConfig);
    })
    .then(function(fileExists) {
      return builder.readOrWriteConfig(fileExists);
    })
    .then(function() {
...
SiteBuilder.prototype._checkInternalPublishingConfiguration = function() {
  ...
  return this.fileHandler.exists('_config_internal.yml')
    .then(function(hasInternalConfig) {
      ...
      return that.fileHandler.exists('_config_external.yml');
    })
