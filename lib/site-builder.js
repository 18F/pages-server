...
//   destDir: directory containing published sites
//
// Returns an object with the following properties:
//   repoDir: input argument prefixed with config.home
...
function Options(info, repoDir, destDir) {
  return {
...
function SiteBuilder(opts, buildLogger, updateLock, doneCallback) {
  ...
  this.done = function(err) {
    if (that.generatedConfig) {
      that.logger.log('removing generated', config.pagesConfig);
      var configPath = path.join(that.opts.sitePath, config.pagesConfig);
...
SiteBuilder.prototype._buildJekyll = function() {
  ...
  return this.checkForFile('Gemfile')
    ...
    .then(function() { return that.checkForFile(config.pagesConfig); })
...
SiteBuilder.prototype.readOrWriteConfig = function(configExists) {
  ...
  if (configExists) {
    this.logger.log('using existing', config.pagesConfig);
    return new Promise(function(resolve, reject) {
  ...
  this.logger.log('generating', config.pagesConfig);
  return new Promise(function(resolve, reject) {
...
exports.launchBuilder = function(info, repositoryDir, generatedSiteDir) {
  var builderOpts = Options(info, repositoryDir, generatedSiteDir);
...
exports.makeBuilderListener = function(webhook, builderConfig) {
  webhook.on('refs/heads/' + builderConfig.branch, function(info) {
    exports.launchBuilder(info, builderConfig.repositoryDir,
